<template>
  <div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
    ...
    <div class="container" v-if="isLoadedTickers">
      <section>
        <div class="flex">
          <div class="max-w-xs">
            ...
            <div class="mt-1 relative rounded-md shadow-md">
              <input
                v-model="ticker"
                @keydown.exact="isAlreadyAdded = false"
                @keydown.enter="
                  add(ticker);
                  fillHints;
                "
      ...
      <section class="relative" v-if="currentTicker !== null">
        <h3 class="text-lg leading-6 font-medium text-gray-900 my-8">
          {{ currentTicker.name }} - USD
        </h3>
        <div class="flex items-end border-gray-600 border-b border-l h-64">
          <div
            v-for="(price, idx) of normalizeGraph()"
            :key="idx"
            :style="{
              height: `${price}%`,
            }"
...
export default {
  ...
  data() {
    return {
      apiKey:
        "ffedf1623b76f30f1798385aeed2f6eefa5dd7038345e91becddf308dfe6e97d",
      ticker: "",
      tickers: [],
      currentTicker: null,
      graph: [],
  ...
  methods: {
    ...
    subscribeToUpdate(tickerName) {
      const interval = setInterval(async () => {
        ...
        if (ticker) {
          ...
          if (!ticker.interval) ticker.intervalId = interval;
        }
    ...
    add(ticker) {
      if (this.checkAlreadyAdded(ticker)) return;
    ...
    remove(tickerToDelete) {
      clearInterval(tickerToDelete.intervalId);
    ...
    normalizeGraph() {
      ...
      const deltaPrice = maxPrice - minPrice === 0 ? 1 : maxPrice - minPrice;

      return this.graph.map((price) => {
        return 5 + ((price - minPrice) * 95) / deltaPrice;
      });
    },

    fillHints() {
      const relevantItems = this.tickersList.filter((ticker) => {
        const regExp = new RegExp(`^${this.ticker}`, "gi");
        const matches =
          ticker.Symbol.match(regExp) || ticker.FullName.match(regExp) || [];
      ...
      while (
        ...
        this.hints.push(relevantItems.pop().Symbol);
      }
    },

    checkAlreadyAdded(ticker) {
      const matches = this.tickers.filter((t) =>
        t.name.match(new RegExp(`^${ticker}$`, "gi"))
      );
    ...
    async getAllTickers() {
      ...
      for (let key in data.Data) {
        this.tickersList.push(data.Data[key]);
      }
      this.isLoadedTickers = true;
    },
  },
  mounted() {
    this.getAllTickers();
